...
var p=setup.puzzle_fifteen,freeslot=[],size=[],m=[],o,o1;
f=document.getElementById("fifteen");
m1=[];
f1=document.getElementById("fifteen1");
ceation_slots();
ceation_slots1();
...
function ceation_slots1(){
    console.log("called")
    size=[p.size[0]/(p.grid[0]+1),p.size[1]/(p.grid[1]+1)]
    var c1=(p.emptySlot)?p.emptySlot:(p.grid[1]+1)*(p.grid[0]+1);
    f1.style.width=p.size[0]+'px';
    f1.style.height=p.size[1]+'px';
    f1.style.position='relative';
    if(p.fill){fifteen_resize1();window.addEventListener('resize',fifteen_resize1,true);}
    o1=1;
    for(var y=0;y<=p.grid[1];y++){
        for(var x=0;x<=p.grid[0];x++){
            if(o1!=c1){
                if(!m1[y]){m1[y]=[]};m1[y][x]=o1;
                var e1=document.createElement("div");
                e1.id="slot1"+o1;
                e1.className="slot1";
                if(p.number){e1.innerHTML=o1}
                e1.style="background-image:url("+p.art.url+");background-size:"+((p.art.ratio)? p.size[0]+"px auto":"auto "+p.size[1]+"px")+";background-position:-"+(size[0]*x)+"px -"+(size[1]*y)+"px ;width:"+size[0]+"px;height:"+size[1]+"px;top:"+(size[1]*y)+"px;left:"+(size[0]*x)+"px;position:absolute;"+((p.style)?p.style:"")
                if(p.time){e1.style.transitionDuration=p.time+"s"}
                f1.appendChild(e1);o1++;
                console.log("loop")
            }else{m1[y][x]=0;o1++;}
        }
    }
...
function check_slots(){
    ...
    }if(check==o){setTimeout(()=>{ alert('You Won The Game') },((p.time)?p.time*1000:0));} // <-- alert('win') script that runs at the end of the game
}
function fifteen_resize1(){
    var rect=f1.parentNode.getBoundingClientRect();
    if((p.size[0]/p.size[1])<(rect.width/rect.height)){f1.style.transform='scale('+(rect.height/p.size[1])+')'}
    else{f1.style.transform='scale('+(rect.width/p.size[0])+')'}
}
